...
void create_user_form() {
  ...
  GtkWidget *create_user_button;
  GtkWidget *vbox;

  window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
  gtk_window_set_title(GTK_WINDOW(window), "Formulaire d'inscription");
  ...
  user_entry_t* user_data = malloc(sizeof(user_entry_t));
...
void login_page(GtkApplication *app) {
  ...
  g_signal_connect(GTK_BUTTON(submit_button), "button-press-event", G_CALLBACK(try_login), NULL);
  ...
  gtk_box_pack_start(GTK_BOX(vbox), gtk_label_new("Password"), 0, 0, 0);
  gtk_box_pack_start(GTK_BOX(vbox), password_field, 0, 0, 0);
  gtk_box_pack_start(GTK_BOX(vbox), submit_button, 0, 0, 0);
  gtk_box_pack_start(GTK_BOX(vbox), create_user_button, 0, 0, 0);
